...
class TrainTieBot:
    ...
    def runDNN(self, query=None, corpus=None):
        ...
        if corpus is None:
            ...
            corpusAnswers=corpusTrain
            corpusQues= corpusTest
        ...
        classes = dnnObject.run()
        answers = self.provideAnswers(classes, corpusAnswers)
        JSONObj = self.convertAnsToJSON(answers)
        ans= JSONObj
        print JSONObj



    def convertAnsToJSON(self, answers):
        for x in answers:
            my_dict = {}

            my_dict['Answer'] = x
            JSONformat= json.dumps(my_dict)
            JSONformat = json.loads(JSONformat)

            return JSONformat


    def provideAnswers(self, classes, corpusAnswers):
        answers = []
        stringAnswers = []
        for x in classes:
            for y in x.values()[0]:
        ...
        print "Answers: "
        for x in answers:
            stringAnswers.append(corpusAnswers['Answer'][int(x)])
...
def main(question):
    ...
    if(question == 'hi' or question == "hello"):
        ...
        answer= "this is a temp ans hahah"
    return answer
