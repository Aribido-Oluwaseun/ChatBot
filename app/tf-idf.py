...
class Similarity:
    ...
    def run_similarity(self):
        ...
        tokenized_docs = [tokenize(d) for d in self.documents]  # tokenized docs
        all_tokens_set = set([item for sublist in tokenized_docs for item in sublist])
        tokenized_query = tokenize(self.query)
        stop_words_removed_docs = self.remove_stopwords(tokenized_docs)
        for i in range(len(stop_words_removed_docs)):
            print(self.jaccard_similarity(tokenized_query, stop_words_removed_docs[i]))
